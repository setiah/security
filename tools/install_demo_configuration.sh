...
function generate_certs() {
  ...
  # Root cert
  openssl genrsa -out root-ca-key.pem 2048 1> /dev/null
  openssl req -new -x509 -sha256 -key root-ca-key.pem -subj "$root_subj" -out root-ca.pem -days $validity 1> /dev/null
  echo -e "Root certificates created at `pwd`/certs/root-ca.pem"

  # Admin cert
  openssl genrsa -out kirk-key-temp.pem 2048 1> /dev/null
  openssl pkcs8 -inform PEM -outform PEM -in kirk-key-temp.pem -topk8 -nocrypt -v1 PBE-SHA1-3DES -out kirk-key.pem 1> /dev/null
  openssl req -new -key kirk-key.pem -subj "$admin_subj" -out kirk.csr 1> /dev/null
  openssl x509 -req -in kirk.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out kirk.pem -days $validity 1> /dev/null
  echo -e "Admin certificates created at `pwd`/certs/kirk.pem"

  # Node cert
  openssl genrsa -out esnode-key-temp.pem 2048 1> /dev/null
  openssl pkcs8 -inform PEM -outform PEM -in esnode-key-temp.pem -topk8 -nocrypt -v1 PBE-SHA1-3DES -out esnode-key.pem 1> /dev/null
  openssl req -new -key esnode-key.pem -subj "$node_subj" -out esnode.csr 1> /dev/null
  openssl x509 -req -in esnode.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out esnode.pem -days $validity 1> /dev/null
